...
class CoutriesOfTheWorld
{
    ...
    public static void Main()
    {
        ...
        int  density =  Int32.Parse(args[2]);
        ...
        var result = from c in Countries
                where c.Continent == theContinent && c.Population / c.Area > density
                orderby c.Population/c.Area descending
                select c;

        foreach(Country c in result)
        {
            Console.WriteLine(String.Format("{0, -25} {1, 15:n0} {2, 15:n0} {3, 10:n1} {2, 15:n0}", 
            	c.Name, c.Population, c.Area, 1.0*c.Population/c.Area));
        }
